...
//Add by linjingming
import net.majorkernelpanic.streaming.Utility;
//Add by linjingming
		...
		synchronized (mInputStream) {
			...
			try {
			    while (mInputStream.available() > 0) {
			        int len = mInputStream.read(buffer, 0, buffer.length);
			        if (len > 0) {
			        	Log.d(TAG,"recreive "+ len);
			        	Log.d(TAG,"buffer:"+ Utility.bytesToHexString(Arrays.copyOf(buffer, len)));
			        }
			    }
			} catch (Exception e) {
				e.printStackTrace();
