	...
	private void sendRequestSetup() throws IllegalStateException, SocketException, IOException {
		for (int i=0;i<2;i++) {
			...
			if (stream != null) {
				...
				*/
				String params = mParameters.transport==TRANSPORT_TCP ? 
						("TCP;unicast;mode=record;interleaved="+2*i+"-"+(2*i+1)) : ("UDP;unicast;client_port="+(5000+2*i)+"-"+(5000+2*i+1)+";mode=receive");
				String request = "SETUP rtsp://"+mParameters.host+":"+mParameters.port+mParameters.path+"/trackID="+i+" RTSP/1.0\r\n" +
				addCSeq() +
				addUserAgent() +
				"Transport: RTP/AVP/"+params+"\r\n" +
				...
				if (mParameters.transport == TRANSPORT_UDP) {
					...
					//Add by linjingming
					stream.setInputStream(mInputStream);
					//Add by linjingming
...
//		String request = "TEARDOWN rtsp://"+mParameters.host+":"+mParameters.port+mParameters.path+" RTSP/1.0\r\n" + addHeaders();
	...
	//Add by linjingming
	private String addCSeq() {
		return "CSeq: " + (++mCSeq) + "\r\n";
	}
	
	private String addSessionID() {
		return (!TextUtils.isEmpty(mSessionID) ? "Session: " + mSessionID + "\r\n":"");
	}
	
	private String addUserAgent() {
		return "User-Agent: libstreaming example-3\r\n";
	}
	//Add by linjingming
